...
fun canonicalizate frags = let
        val canon = traceSchedule o basicBlocks o linearize
        fun aux [] procs strings = (procs, strings)
        | aux (x::xs) procs strings = case x of
                tigerframe.PROC {body,frame} => (aux xs) ((canon body, frame)::procs) strings 
                | tigerframe.STRING s => (aux xs) procs (s::strings)
    in
        aux frags [] []
    end

fun printcanon (procs,strings) = let
        fun printstrings [] = ()
          | printstrings ((lab,str)::xs) = (print (lab ^ ": " ^ str ^ "\n"); printstrings xs)
        fun printprocs [] = ()
          | printprocs ((stms,frame)::xs) = let
                val _ = print ((tigerframe.name frame)^":\n")
                val _ = map (print o tigerit.tree) stms
                val _ = print "-----------------------------\n"
            in
                printprocs xs
            end
